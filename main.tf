...
resource "aws_security_group_rule" "ingress" {
  security_group_id = aws_security_group.sg_frontend_terraform
  type              = "ingress"
...
resource "aws_security_group_rule" "ingress" {
  ...
  from_port   = var.var.allowed_ingress_ports_backend[count.index]
  to_port     = var.var.allowed_ingress_ports_backend[count.index]
  protocol    = "tcp"
  cidr_blocks = ["0.0.0.0/0"]
}
resource "aws_security_group_rule" "ingress" {
  security_group_id = aws_security_group.sg_backend_terraform
  type              = "ingress"
...
resource "aws_eip" "ip_elastica_backend" {
  instance = aws_instance.instancia_terraform_backend
}

resource "aws_eip" "ip_elastica_frontend" {
  instance = aws_instance.instancia_terraform_frontend
}